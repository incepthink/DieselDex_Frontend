...
import type { CurrencyBoxMode, SwapState } from "@/app/swap/SwapForm";
...
const useSwap = ({ swapState, mode, slippage, pools }: Props) => {
  ...
  const getTxCost = useCallback(async () => {
    ...
    const [simulatedSellAmount, simulatedBuyAmount] =
      mode === "sell"
    ...
    const buyAmountWithSlippage = simulatedBuyAmount[1]
      .mul(bn(10_000).sub(bn(slippage)))
      .div(bn(10_000));
    const sellAmountWithSlippage = simulatedSellAmount[1]
      .mul(bn(10_000).add(bn(slippage)))
      .div(bn(10_000));

    const tx =
      mode === "sell"
        ? await miraDex.swapExactInput(
            sellAmount,
            sellAssetIdInput,
