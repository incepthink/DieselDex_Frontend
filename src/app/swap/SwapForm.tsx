...
const SwapForm: React.FC = () => {
  ...
  const sellMetadata = useMemo(
    () =>
      assets.find((a) => a.assetId === swapState.sell.assetId) as
        | Asset
        | undefined,
  ...
  const buyMetadata = useMemo(
    () =>
      assets.find((a) => a.assetId === swapState.buy.assetId) as
        | Asset
        | undefined,
  ...
  try {
    const decimals = sellMetadata?.decimals ?? 0;
    const insufficientSellBalance = sellBalanceValue.lt(
      bn.parseUnits(sellValue, decimals)
    );
